...
export async function generateInferredSelectClause(
  ...
  const sampleQuery = await connection.query(`
    SELECT * FROM read_csv_auto('${fileName}', header=true, all_varchar=true, sample_size=100) LIMIT 100
  `);
  ...
  return columns.map(col => {
    ...
    if (colSamples.length === 0) return `"${col}"`;
    ...
    if (dateMatches / colSamples.length > 0.7) {
      return `COALESCE(
        ...
        try_strptime("${col}", ${DATE_FORMATS})::DATE
      ) AS "${col}"`;
    }

    const numMatches = colSamples.filter((s: any) => {
      const cleaned = s.replace(/[,%]/g, "");
      return !isNaN(Number(cleaned)) && cleaned !== "";
    }).length;

    if (numMatches / colSamples.length > 0.7) {
      return `try_cast(regexp_replace("${col}", '[,%]', '', 'g') AS DOUBLE) AS "${col}"`;
    }
